...
- The outer connection task is spawned with `tokio::spawn` and its `JoinHandle` is stored on `App.connection_handle`. A shared `CancellationToken` (`App.connection_cancel`) is passed into telnet/SSH so every per-connection subtask (reader, writer, handshake) exits on disconnect — don't spawn fire-and-forget tasks that only watch the socket. `cancel_connection()` cancels the token, sends `Disconnect`, then aborts the outer handle as a backstop.
